...
static void print_usage(const char *prog) {
    printf("Usage: %s [options] <zippath> <outpath>\n", prog);
    printf("\n");
    printf("Convert FAA Aeronav charts to web map tiles.\n");
    printf("\n");
    printf("Arguments:\n");
    printf("  zippath              Directory containing downloaded ZIP files\n");
    printf("  outpath              Output directory for tiles\n");
    printf("\n");
    printf("Options:\n");
    printf("  -c, --config <path>  Config file (default: datasets.json)\n");
    printf("  -t, --tmpdir <path>  Temp directory (default: /tmp/aeronav2tiles)\n");
    ...
    printf("  -T, --threads <N>    Threads per warp operation (default: auto)\n");
    ...
    printf("Examples:\n");
    printf("  %s ./zips ./tiles\n", prog);
    printf("  %s --tilesets sec,tac ./zips ./tiles\n", prog);
    printf("  %s --jobs 4 --threads 12 ./zips ./tiles\n", prog);
}
...
int main(int argc, char *argv[]) {
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "c:t:s:f:j:T:w:rqlh", long_options, NULL)) != -1) {
        switch (opt) {
            case 'c':
                config_path = optarg;
                break;
    ...
    /* Remaining arguments: zippath outpath */
    if (optind + 2 != argc) {
        error("Required arguments: <zippath> <outpath>");
        print_usage(argv[0]);
        return 1;
    }
    opts.zippath = argv[optind];
    ...
    if (opts.threads == 0) {
        opts.threads = cpu_count / opts.jobs;
    ...
    info("  zippath: %s", opts.zippath);
    info("  outpath: %s", opts.outpath);
    info("  tmppath: %s", opts.tmppath);
    info("  CPUs: %d, jobs: %d, threads/job: %d, tile workers: %d",
         cpu_count, opts.jobs, opts.threads, opts.tile_workers);

    /* Ensure output and temp directories exist */
    if (mkdir_p(opts.outpath) != 0) return 1;
